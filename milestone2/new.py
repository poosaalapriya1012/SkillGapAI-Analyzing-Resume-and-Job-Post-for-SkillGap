...
from visuals import create_skill_radar, create_skill_treemap, create_skill_sunburst
...
class CompleteSkillExtractionApp:
    ...
    def _visualization_tab(self):
        ...
        # Additional diagrams: radar and treemap (non-destructive additions)
        st.markdown("---")
        st.subheader("Radar: Skills by Major Category")
        try:
            radar_fig = create_skill_radar(result['categorized_skills'])
            st.plotly_chart(radar_fig, use_container_width=True)
        except Exception as e:
            st.error(f"Failed to render radar chart: {e}")

        st.subheader("Treemap: Categories and Sample Skills")
        try:
            treemap_fig = create_skill_treemap(result['categorized_skills'])
            st.plotly_chart(treemap_fig, use_container_width=True)
        except Exception as e:
            st.error(f"Failed to render treemap: {e}")

        # Sunburst (interactive, categories -> skills)
        st.subheader("Interactive Sunburst: Categories → Skills")
        try:
            sunburst_fig = create_skill_sunburst(result['categorized_skills'])
            st.plotly_chart(sunburst_fig, use_container_width=True)
        except Exception as e:
            st.error(f"Failed to render sunburst: {e}")
